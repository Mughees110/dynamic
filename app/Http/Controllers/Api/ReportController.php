...
class ReportController extends Controller
{
    ...
    public function store(Request $request){
        try {
            ...
            $form=new Report;
            $form->result=$request->json('result');
    ...
    public function update(Request $request){
        try {
            ...
            $form=Report::find($request->json('parameterId'));
            if(!$form){
                return response()->json([
    ...
    public function delete(Request $request){
    	if(empty($request->json('reportId'))){
    		return response()->json([
                    'message' => 'All fields (reportId) required',
                ], 422);
    	}
    	$form=Report::find($request->json('parameterId'));
    	if(!$form){
